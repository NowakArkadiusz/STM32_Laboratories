...
void ADC3_IRQHandler(void)
{
	...
	{
		val = ADC3->DR;
		voltage = 3.0f * (float)val / 4095.0f;
	}
}
volatile uint16_t tab2[2];
void ADC_Conf_Dma_2Channels(void)
{
  ...
  DMA1_Channel3->CMAR 	= (uint32_t)tab2;
	DMA1_Channel3->CNDTR 	= (uint16_t)2;
	DMA1_CSELR->CSELR			= 0;
	DMA1_Channel3->CCR 		|= DMA_CCR_CIRC | DMA_CCR_MINC 
												| DMA_CCR_PSIZE_0 | DMA_CCR_MSIZE_0 | DMA_CCR_EN;
	
	ADC123_COMMON->CCR |= ADC_CCR_TSEN;
	ADC3->CR = ADC_CR_ADEN | ADC_CR_ADVREGEN;
	ADC3->SQR1 = (1<<0) | (3<<6) | (17<<12);//Dwie konwersje, kanal 3 i kanal 17
	ADC3->SMPR1 = ADC_SMPR1_SMP3;
